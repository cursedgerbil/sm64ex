    ...
    if (gCurrentArea->paintingWarpNodes != NULL && gMarioState->floor != NULL) {
        ...
        if (pWarpNode != NULL) {
            if (gMarioState->action & ACT_FLAG_INTANGIBLE) {
                ...
                s16 destCourse = gLevelToCourseNumTable[warpNode.destLevel - 1];		// Only check for paintings when in the castle, otherwise they could be interior warps and we don't want to block those
